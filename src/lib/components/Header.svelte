...
<header
	class={["fixed", "top-0", "right-0", "left-0", "z-50", "flex", "flex-col", "px-5", "text-white",
	"transition-all", "duration-300", "ease-in-out", "md:px-10",
	{ 'bg-black shadow-lg': isSticky, 'bg-linear-to-b from-[rgba(0,0,0,0.7)] to-transparent': !isSticky }
]}
>
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center space-x-4">
			<div
				id="search-bar"
				class={["hidden", "md:flex", "search-container", {active: isSearchActive}]}
			...
			<button id="ham-button" class="md:hidden ml-4 focus:outline-none" aria-label="hamburger-button" onclick={toggleMenu}>
				<Menu size={20} color="white" />
			</button>
		</div>
	</div>

	<div id="mobile-menu" class={["mobile-menu", "relative", "lg:hidden", {"open": isMenuOpen}]} role="presentation">
	...
	{#if isMenuOpen}
		<div class="overlay show" onclick={closeMenu} role="presentation">
...
<style>
 ...
 .search-input {
	...
	.mobile-menu {
		...
		padding: 1rem 2rem;
		transition: trasnsoform .3s ease-in-out;
